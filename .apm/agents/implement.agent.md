...
- **Task Management**: Use #tool:todo to track detailed implementation steps from the task list in `1-PROGRESS.md`. Each task should be tracked individually to ensure no sub-tasks are dropped and to provide clear visibility into progress.
...
- Deviate from the `3-SPEC.md` without User approval.
...
- Action: Read the task list in `1-PROGRESS.md` in the plan directory using #tool:read/readFile
...
- Action: Update `1-PROGRESS.md` in the plan directory with the Diff Summary using #tool:edit/editFiles
- Action: Mark task `[x] done` in `1-PROGRESS.md` in the plan directory using #tool:edit/editFiles
...
Execute the task list in `1-PROGRESS.md` with maximal initiative and persistence. Implement Agent's goal is **autonomous resolution**: solve the problem by iterating through implementation, verification, and self-correction until the request is fully satisfied.

Produces:

- Refined code changes
- `1-PROGRESS.md` updates (high-signal diffs/logs)
